...
---
```{r include = FALSE}
# Show warnings in markdown output, as they may signal changes in data quality or point to unexpected behaviors
...
library(httr2) # Perform HTTP requests (in this case used to get data from FACT API)
...
api_key <- Sys.getenv("API_KEY_EIA") # This expects users to have an environment variable API_KEY_EIA in a .Renviron file locally.
eia_860_year <- 2024 # The  EIA data year (available at https://www.eia.gov/electricity/data/eia860/)
earliest_retirement_year <- 2010 # All units retired before this year are filtered out of the data

# Try first at normal location; if we get a 301, try archive instead
eia_data_file <- str_glue("https://www.eia.gov/electricity/data/eia860/xls/eia860{eia_860_year}.zip")
check_available_status <- request(eia_data_file) |> req_method("HEAD") |> req_options(followlocation=FALSE) |> req_perform() |> resp_status()
if (check_available_status == 301) {
  # bc eia.gov's 301 points to a nonsense URL we have to do our own redirect
  eia_data_file <- str_glue("https://www.eia.gov/electricity/data/eia860/archive/xls/eia860{eia_860_year}.zip")
}
check_available_status <- request(eia_data_file) |> req_method("HEAD") |> req_options(followlocation=FALSE) |> req_perform() |> resp_status()
if (check_available_status != 200) {
  stop(paste("Year",eia_860_year,"is not available."))
}
...
response <- req_perform(request(str_glue(
...
camd_plants_json <- as.tbl_json(resp_body_string(response)) %>%
...
get_unmatched <- function(original_data, prev_matches, by = NULL) {
  if (missing(prev_matches) || all(is.na(prev_matches)) || is.null(prev_matches)) {
